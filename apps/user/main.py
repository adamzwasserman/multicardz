...
from .routes.group_tags_api import router as group_tags_router
...
def create_app():
    ...
    static_ttl = int(
        os.getenv("MULTICARDZ_STATIC_CACHE_TTL", "60" if is_dev else "31536000")
    )
        ...
        def file_response(self, *args, **kwargs):
            ...
            if not is_dev:
                response.headers["Cache-Control"] = (
                    f"public, max-age={static_ttl}, immutable"
                )
            else:
    ...
    async def read_root(request: Request):
        try:
            ...
            try:
                with sqlite3.connect(DATABASE_PATH) as conn:
                    ...
                    print(
                        f"DEBUG: Loading {len(available_tags)} tags from tutorial database: {available_tags}"
                    )
                ...
                logger.info(
                    f"Fallback to lesson {current_lesson} tags: {available_tags}"
                )
        ...
        try:
            with sqlite3.connect(
                "/Users/adam/dev/multicardz/data/multicardz_dev.db"
            ) as conn:
                cursor = conn.cursor()
                cursor.execute(
